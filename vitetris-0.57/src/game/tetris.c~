...
struct player plr_self; //williamiced
struct player plr_self_next; //williamiced
struct tetr Next_tetris; //williamiced
struct tetr Save_tetris; //williamiced
...
static int nextpiece(struct tetr *next)
{
	...
	Next_tetris = *next; //williamiced
	plr_self_next = player1; //williamiced
...
int draw_my_self()
{
	drawsave(&plr_self, &Save_tetris);
	drawnext(&plr_self_next, &Next_tetris);
}

int makeSwitch(struct player* plr)
{
	static int HaveSave = 0;
	static struct tetr tmpPiece;
	static struct tetr drawPiece;
	static struct tetr SavePiece;
	int i;

	clearblocks(plr, 10, 10, 10);

	if(save_coda == 0)
		return 1;

	if((plr -> piece).blocks == TETR_I ||(plr -> piece).blocks == TETR_I2)
		i = 0;
	else if((plr -> piece).blocks == TETR_J ||(plr -> piece).blocks == TETR_J2 || 
		(plr -> piece).blocks == TETR_J3 ||(plr -> piece).blocks == TETR_J4)
		i = 1;
	else if((plr -> piece).blocks == TETR_L ||(plr -> piece).blocks == TETR_L2 || 
		(plr -> piece).blocks == TETR_L3 ||(plr -> piece).blocks == TETR_L4)
		i = 2;
	else if((plr -> piece).blocks == TETR_O)
		i = 3;
	else if((plr -> piece).blocks == TETR_S ||(plr -> piece).blocks == TETR_S2)
		i = 4;
	else if((plr -> piece).blocks == TETR_T ||(plr -> piece).blocks == TETR_T2 || 
		(plr -> piece).blocks == TETR_T3 ||(plr -> piece).blocks == TETR_T4)
		i = 5;
	else if((plr -> piece).blocks == TETR_Z ||(plr -> piece).blocks == TETR_Z2)
		i = 6;
	else
		i = 7;
	gettetrom(&drawPiece, i);

	if(HaveSave == 0)
	{
		clearblocks(plr, (plr->piece).blocks, (plr->piece).x, (plr->piece).y);
		SavePiece.blocks = drawPiece.blocks;
		SavePiece.color = drawPiece.color;
		HaveSave = 1;
		drawsave(plr, &drawPiece); //williamiced
		Save_tetris = drawPiece; // williamiced
		plr_self = *plr; // williamiced
		return 0;
	}
	else
	{
		...
		tmpPiece.blocks = drawPiece.blocks;
		tmpPiece.color = drawPiece.color;
		(plr->piece).blocks = SavePiece.blocks;
		(plr->piece).color = SavePiece.color;
		SavePiece.blocks = tmpPiece.blocks;
		SavePiece.color = tmpPiece.color;

		if((plr -> piece).blocks == TETR_I ||(plr -> piece).blocks == TETR_I2)
			i = 0;
		else if((plr -> piece).blocks == TETR_J ||(plr -> piece).blocks == TETR_J2 || (plr -> piece).blocks == TETR_J3 ||(plr -> piece).blocks == TETR_J4)
			i = 1;
		else if((plr -> piece).blocks == TETR_L ||(plr -> piece).blocks == TETR_L2 || (plr -> piece).blocks == TETR_L3 ||(plr -> piece).blocks == TETR_L4)
			i = 2;
		else if((plr -> piece).blocks == TETR_O)
			i = 3;
		else if((plr -> piece).blocks == TETR_S ||(plr -> piece).blocks == TETR_S2)
			i = 4;
		else if((plr -> piece).blocks == TETR_T ||(plr -> piece).blocks == TETR_T2 || (plr -> piece).blocks == TETR_T3 ||(plr -> piece).blocks == TETR_T4)
			i = 5;
		else if((plr -> piece).blocks == TETR_Z ||(plr -> piece).blocks == TETR_Z2)
			i = 6;
		else
			i = 7;
		gettetrom(&(plr->piece), i);
		(plr->piece).y += 2;
		drawpiece(plr, (plr->piece).blocks, (plr->piece).x, (plr->piece).y);
		drawsave(plr, &drawPiece);
		Save_tetris = drawPiece; //williamiced
		plr_self = *plr; // williamiced
		save_coda = 0;
		return 1;
